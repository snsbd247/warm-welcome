...
export default function SupplierPayments() {
  ...
  const { data: suppliers = [] } = useQuery({ queryKey: ["suppliers"], queryFn: async () => { const { data } = await apiDb.from("suppliers").select("*"); return data || []; } });
  const { data: rows = [], isLoading } = useQuery({ queryKey: ["supplier_payments"], queryFn: async () => { const { data } = await apiDb.from("supplier_payments").select("*").order("paid_date", { ascending: false }); return data || []; } });

  const save = useMutation({
    mutationFn: async () => { await apiDb.from("supplier_payments").insert({ ...form, amount: Number(form.amount), paid_date: new Date().toISOString() }); },
