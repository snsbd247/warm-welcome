...
export default function DailyAttendance() {
  ...
  const { data: employees = [] } = useQuery({ queryKey: ["employees-active"], queryFn: async () => { const { data } = await apiDb.from("employees").select("*").eq("status", "active").order("employee_id"); return data || []; } });

  useQuery({
    queryKey: ["attendance", selectedDate],
    queryFn: async () => {
      const { data } = await apiDb.from("attendance").select("*").eq("date", selectedDate);
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      for (const emp of employees) {
        const r = getR(emp.id);
        await apiDb.from("attendance").upsert({ employee_id: emp.id, date: selectedDate, status: r.status, check_in: r.check_in || null, check_out: r.check_out || null });
      }
    },
