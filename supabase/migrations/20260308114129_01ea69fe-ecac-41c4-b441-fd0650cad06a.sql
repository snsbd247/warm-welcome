...
DROP POLICY IF EXISTS "Admins can view bills" ON public.bills;
DROP POLICY IF EXISTS "Anon can read bills" ON public.bills;
DROP POLICY IF EXISTS "Anon can read bills by token" ON public.bills;
...
CREATE POLICY "Anyone can read packages" ON public.packages FOR SELECT TO anon, authenticated
USING (true);
...
DROP POLICY IF EXISTS "Anon can read own tickets" ON public.support_tickets;
